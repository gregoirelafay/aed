function [config, store, obs] = aev11features(config, setting, data)         
% aev11features FEATURES step of the expLanes experiment aed_v1              
%    [config, store, obs] = aev11features(config, setting, data)             
...
if nargin==0, aed_v1('do', 1:3, 'mask', {0 0 1 0 [1 4] 0 0 0 0},'parallel',4); return; else store=[]; obs=[]; end

store.xp_settings.hoptime = setting.hoptime;
store.xp_settings.wintime = setting.hoptime*2;
...
switch setting.features
    case 'spec'
        [~,~,store.features] = melfcc(y,store.xp_settings.sr, 'wintime',store.xp_settings.wintime,'hoptime',store.xp_settings.hoptime,'nbands',40,'minfreq',0,'maxfreq',12000,'preemph',0,'useenergy',1,'lifterexp',0);
    case 'mel'
        [~,store.features,~] = melfcc(y,store.xp_settings.sr, 'wintime',store.xp_settings.wintime,'hoptime',store.xp_settings.hoptime,'nbands',40,'minfreq',0,'maxfreq',12000,'preemph',0,'useenergy',1,'lifterexp',0);
